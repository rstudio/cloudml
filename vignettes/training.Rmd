...
CloudML is a managed service where you pay only for the hardware resources that you use. Prices vary depending on configuration (e.g. CPU vs. GPU vs. multiple GPUs). See <https://cloud.google.com/vertex-ai> for additional details.
...
You can experiment with hyperparameters on an ad-hoc basis, but in general it's better to explore them more systematnically. The key to doing this with CloudML is by defining [training flags](https://tensorflow.rstudio.com/guides/tfruns/tuning) within your script and the parameterizing runs using those flags. 
...
For example, the following would train the same model as above but with a [Tesla K80 GPU](https://resources.nvidia.com/l/en-us-gpu):
...
See the CloudML website for documentation on [available machine types](https://cloud.google.com/vertex-ai). Also note that GPU instances can be considerably more expensive that CPU ones! See the documentation on [CloudML Pricing](https://cloud.google.com/vertex-ai) for details.
...
Note that `trainingInput` is used as the top level key in the config file (this is required). Additional documentation on available fields in the configuration file is available here <https://cloud.google.com/vertex-ai>.
