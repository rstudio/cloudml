...
```{r echo=FALSE}
config_yml <- system.file("examples/census/cloudml.yml", package = "cloudml")
# config_yml <- system.file("examples/mtcars/cloudml.yml", package = "cloudml")
# config_yml <- system.file("examples/mnist/cloudml.yml", package = "cloudml")

result <- file.copy(config_yml, "cloudml.yml")

config <- gcloud_config()
if (config$account == "account@domain.com") stop("Please configure 'cloudml.yml'")

contents <- readLines(config_yml)
writeLines(contents)

result <- file.copy("cloudml.yml", config_yml, overwrite = T)
```
